...
if ($method === 'GET') {
    $sql = "SELECT id, title, DATE_FORMAT(date, '%Y-%m-%d') as date, time, duration, location, type, participants, description, status FROM training_events ORDER BY date ASC";
    ...
    if ($stmt->execute()) {
        echo json_encode(['id' => $stmt->insert_id, 'message' => 'TDS event added']);
    } else {
    ...
    if ($stmt->execute()) {
        echo json_encode(['message' => 'TDS event updated']);
    } else {
        echo json_encode(['error' => $stmt->error]);
    }
    ...
    if ($stmt->execute()) {
        echo json_encode(['message' => 'TDS event deleted']);
    } else {
        echo json_encode(['error' => $stmt->error]);
    }
}
